# GEMINI.md - GC's View (Synced from CLAUDE.md)

**Last Synced**: 2026-01-16 18:15 EET
**Sync Source**: CLAUDE.md v2.5.2
**Agent**: GC (Gemini CLI)

Version: 2.5.2 | Last Updated: 2026-01-16 | Agent: GC | Status: ACTIVE

***

## TABLE OF CONTENTS

1. GC Operating Instructions (MANDATORY - READ FIRST)
...
### Core Identity
- GC (Gemini CLI): 1M context window, large refactors, git operations
...
**NEW - Designed by CC, Implemented by GC in scripts/pr-scrape.ts**
...
- ✅ R-004: pnpm CI environment - **RESOLVED via PR #81**
...
## 8. SESSION TIMELINE (GC Perspective)
...
- **2026-01-16**: Enhanced Scraper (5D Taxonomy) - CC specification, GC implementation
...
## 9.5. GC-SPECIFIC WORKFLOWS

### Large Context Window Usage (1M Tokens)
- When to use: Repository-wide refactors, mass file migrations, comprehensive audits
- How: `grep -r "pattern" . | wc -l` to estimate token count first
- Limit: Process max 500 files per session to avoid timeout

### GitHub API Pagination Patterns
- Use `gh api --paginate` for fetching large lists (branches, PRs)
- Always check rate limits `gh api rate_limit` before intensive operations

### Git Conflict Resolution Recipes
- **Docs Conflict**: Always favor `CLAUDE.md` content, append new info
- **Code Conflict**: `git checkout --ours` (local) vs `git checkout --theirs` (remote), then manual merge
...
**END OF GEMINI.md v2.5.2**

Maintained By: GC (synced from CC) | Line Target: ~500
