...
class QualityGate:
    ...
    def validate(self, report: Dict) -> Tuple[bool, List[str]]:
        ...
        # Check for merged cell artifacts
        merged_artifacts = [
            u for u in report['unknowns'] 
            if len(u['label_en'].split()) > 5 and u['trim_count'] > 0
        ]
        if merged_artifacts:
            issues.append(f"Possible merged cell artifacts: {len(merged_artifacts)} rows")
        
        # Check for empty labels with trim data
        empty_with_data = [
            m for m in report['matches'] + report['unknowns']
            if not m['label_en'].strip() and m['trim_count'] > 0
        ]
        if empty_with_data:
            issues.append(f"Empty labels with trim data: {len(empty_with_data)} rows")
    ...
    def generate_report(self, analysis_report: Dict) -> Dict:
        ...
        report = {
            ...
            'recommendations': self._generate_recommendations(analysis_report, duplicate_stats, high_conf_unknowns)
        }
    ...
    def _generate_recommendations(
        ...
        if duplicate_stats:
            recommendations.append(
                f"Review {len(duplicate_stats)} duplicate specs - may indicate table structure issues"
            )
        ...
        if analysis_report['coverage_percent'] < 50:
            recommendations.append(
                "Coverage below 50% - consider expanding spec definitions or reviewing extraction quality"
            )
