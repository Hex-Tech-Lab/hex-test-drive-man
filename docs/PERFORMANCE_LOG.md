## 2026-01-07 1840 UTC - BB - BUG-011 Proper Fix (Drawer Flash)
**Timebox**: 30 minutes (planned)
**Start**: 2026-01-07 1840 UTC
**End**: 2026-01-07 1700 UTC
...
**Summary**:
Fixed BUG-011 transparent drawer flash on page reload. Previous fix (PR #45) attempted to hide skeleton but didn't address root cause.

**Root Cause Identified**:
- CartDrawerSkeleton had `open={true}` hardcoded
- Skeleton was always visible during SSR/hydration even when drawer should be closed
- Skeleton was rendered as loading fallback in `dynamic()` import, causing flash on every page load

**Solution Implemented**:
1. CartDrawerSkeleton now accepts `open` and `onClose` props (defaults to `open=false`)
2. Header.tsx only renders CartDrawer when `cartDrawerOpen=true` using conditional rendering
3. Uses Suspense with skeleton fallback that only shows when drawer is actually opening
4. Removed skeleton from `dynamic()` loading prop to prevent SSR flash

**Files Changed**:
- src/components/Header.tsx: Conditional rendering with Suspense
- src/components/skeletons/CartDrawerSkeleton.tsx: Added props interface

**Testing Completed**:
✅ Hard reload on /ar route: No transparent flash
✅ Hard reload on /en route: No transparent flash
✅ Click cart icon: Drawer opens smoothly with skeleton during lazy load
✅ Production build tested locally (`pnpm build && pnpm start`)
✅ Works on both RTL (Arabic) and LTR (English) layouts
✅ Build passes: `pnpm build` successful

**PR Created**: #47
**Branch**: bb/bug-011-drawer-flash-proper-fix
**Commit**: 34b97fe
